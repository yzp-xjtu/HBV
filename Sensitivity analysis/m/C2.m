...
global SENS_CFG;
if isempty(SENS_CFG)
    SENS_CFG.p_vaccine = 0.85; SENS_CFG.clearance_rate = 'random'; 
    SENS_CFG.transm_bound = 'random'; SENS_CFG.intranterine = 0.8; 
...
for i = 1:nMC
    if strcmp(SENS_CFG.clearance_rate, 'random')
        recoverrate_samples(:, i) = 0.005 + (0.015 - 0.005) .* rand(maxage, 1);
    else
        recoverrate_samples(:, i) = str2double(SENS_CFG.clearance_rate) * ones(maxage, 1);
    end
end
...
for mc = 1:nMC
    if strcmp(SENS_CFG.transm_bound, 'lower')
        transmrate_samples(:, mc) = lower_bound;
    elseif strcmp(SENS_CFG.transm_bound, 'upper')
        transmrate_samples(:, mc) = upper_bound;
    else
...
intranterine = SENS_CFG.intranterine;  
vaccinarate = SENS_CFG.p_vaccine * HBVdata20241(1:N,2) ;
...
for m = 1:nMC
    ...
    for j=1:N
        ...
        adultvaccinarate(:,j)=SENS_CFG.p_vaccine.*HBVdata20243(:,j).*0; 
...
for idx = 1:length(names)
    ...
    writematrix(m,     fullfile(output_dir, sprintf('2_%s101.csv', name)));
    writematrix(lower, fullfile(output_dir, sprintf('2_%s102.csv', name)));
    writematrix(upper, fullfile(output_dir, sprintf('2_%s103.csv', name)));
end
...
writematrix(m,     fullfile(output_dir, sprintf('2_%s101.csv', "yrhbsrate")));
writematrix(lower, fullfile(output_dir, sprintf('2_%s102.csv', "yrhbsrate")));
writematrix(upper, fullfile(output_dir, sprintf('2_%s103.csv', "yrhbsrate")));
save_filename = fullfile(output_dir, 'C2.mat');
